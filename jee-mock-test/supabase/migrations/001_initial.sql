-- ════════════════════════════════════════════════════════════════════
-- JEE Mock Test Generator — Supabase Migration
...
CREATE TABLE IF NOT EXISTS questions (
  ...
  options         JSONB,           -- [{label: 'A', text: '...'}, ...]  null for numerical
  correct_answer  TEXT NOT NULL,   -- 'A','B','C','D' or numerical value
  explanation     TEXT,
...
CREATE TABLE IF NOT EXISTS attempts (
  ...
  percentage      NUMERIC,
  subject_scores  JSONB,  -- {physics: {score, max, correct}, ...}
  status          TEXT NOT NULL DEFAULT 'in_progress' CHECK (status IN ('in_progress', 'submitted', 'abandoned'))
);

-- ─── INDEXES ─────────────────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_tests_clerk_user     ON tests(clerk_user_id);
...
CREATE INDEX IF NOT EXISTS idx_questions_number      ON questions(test_id, question_number);
...
-- Tests: owner can do anything; public tests readable by all
CREATE POLICY "tests_owner_all"
  ON tests FOR ALL
...
-- Questions: readable if test is accessible
  ...
  USING (
    EXISTS (
      ...
      WHERE tests.id = questions.test_id
        AND tests.clerk_user_id = current_setting('request.jwt.claims', true)::json->>'sub'
    )
  );
...
-- Test owners can read all attempts for their tests
CREATE POLICY "attempts_test_owner_read"
...
-- supabase storage create diagrams --public
...
CREATE POLICY "diagrams_owner_upload"
  ON storage.objects FOR INSERT
  WITH CHECK (bucket_id = 'diagrams');

CREATE POLICY "diagrams_owner_delete"
