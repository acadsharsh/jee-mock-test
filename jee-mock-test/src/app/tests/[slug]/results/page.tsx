...
export default async function ResultsPage({ params, searchParams }: Props) {
  ...
  const [{ data: attempt }, { data: test }] = await Promise.all([
    supabase.from("attempts").select("*").eq("id", attemptId).eq("clerk_user_id", userId).single(),
    supabase.from("tests").select("*, questions(*)").eq("slug", params.slug).single(),
  ]);

  if (!attempt || !test) notFound();

  const questions = ((test.questions as Question[]) || []).sort(
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="card-neo p-0 overflow-hidden mb-8">
        <div className="bg-ink-900 px-8 py-10 text-center">
          <div className={`font-display text-8xl font-black mb-2 ${scoreColor.replace("text-", "text-")}`} style={{color: pct >= 75 ? "#10B981" : pct >= 50 ? "#F59E0B" : "#DC2626"}}>
            {pct.toFixed(1)}%
          </div>
          <div className="font-mono text-ink-300 text-sm mb-1">{badge}</div>
          <div className="font-display text-2xl text-ink-50 font-bold">
                ...
                <div key={topic} className="px-5 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 bg-ink-100 h-2 border border-ink-200 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.max(0, Math.min(100, topicPct))}%`,
                        backgroundColor: topicPct >= 75 ? "#10B981" : topicPct >= 50 ? "#F59E0B" : "#DC2626",
                      }}
                    />
                  </div>
                  <span className="font-mono text-xs text-ink-600 w-16 text-right">
                    {data.score}/{data.max} ({topicPct.toFixed(0)}%)
                  </span>
                </div>
              );
            })}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {q.options.map((opt) => {
                          ...
                          return (
                            <div
                              key={opt.label}
                              className={`px-3 py-2 border text-xs flex items-start gap-2 ${
                                isCorrectOpt ? "border-emerald-500 bg-emerald-50" :
                                isUserPick && !isCorrectOpt ? "border-crimson-500 bg-red-50" :
      ...
      <div className="mt-8 flex gap-3">
        <Link href={`/tests/${params.slug}`} className="btn-neo-outline">
          <ArrowLeft size={15} /> Back to Test
        </Link>
        <Link href={`/tests/${params.slug}/attempt`} className="btn-neo">
