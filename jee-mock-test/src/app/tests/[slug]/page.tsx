...
import { Clock, BookOpen, Globe, Lock, Play, BarChart3, Share2 } from "lucide-react";
...
export default async function TestPage({ params }: Props) {
  ...
  const isOwner = test.clerk_user_id === userId;
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          ...
          {test.is_public ? (
            <span className="badge text-emerald-600 border-emerald-600 flex items-center gap-1">
              <Globe size={10} /> Public
            </span>
          ) : (
            <span className="badge text-ink-400 border-ink-300 flex items-center gap-1">
        ...
        {test.description && (
          <p className="text-ink-500 font-body text-sm">{test.description}</p>
        )}
      ...
      <div className="grid grid-cols-4 gap-0 border-2 border-ink-900 bg-white mb-8">
        {[
          ...
          { label: "Marks/Question", value: "4 / −1", icon: BarChart3 },
        ].map((s, i) => {
          const Icon = s.icon;
          return (
      ...
      <div className="flex flex-wrap gap-3 mb-10">
        <Link
          href={`/tests/${params.slug}/attempt`}
          className="btn-neo-amber px-6 py-2.5"
        >
