      ...
      <div className="grid grid-cols-3 gap-0 border-2 border-ink-900 mb-8">
        {stats.map((s, i) => {
          ...
          return (
            <div
              key={i}
              className={`px-8 py-6 bg-white ${i < 2 ? "border-r-2 border-ink-900" : ""}`}
            >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card-neo p-0 overflow-hidden">
          <div className="px-6 py-4 border-b-2 border-ink-900 flex items-center justify-between">
            ...
            <Link href="/dashboard/tests" className="text-xs font-mono text-ink-500 hover:text-ink-900 transition-colors">
              View all →
            </Link>
          </div>
          {tests && tests.length > 0 ? (
            <div>
              {(tests as Test[]).map((test, i) => (
                <div
                  key={test.id}
                  className={`px-6 py-4 flex items-center justify-between hover:bg-ink-50 transition-colors ${
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ink-400 font-mono mt-0.5">
                      {test.subject} · {formatDate(test.created_at)}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    ...
                    <Link href={`/tests/${test.slug}`} className="btn-neo-outline text-xs px-3 py-1">
                      View
                    </Link>
                  </div>
                </div>
            ...
            <div className="px-6 py-12 text-center">
              ...
              <Link href="/tests/new" className="btn-neo text-xs">
            ...
            <div>
              {allAttempts.map((attempt, i) => (
                <div
                  key={attempt.id}
                  className={`px-6 py-4 flex items-center justify-between hover:bg-ink-50 transition-colors ${
                    i < allAttempts.length - 1 ? "border-b border-ink-100" : ""
                  }`}
                >
                  <div className="min-w-0">
                    <p className="font-body font-medium text-sm text-ink-900 truncate">
                      {attempt.tests?.title}
                    </p>
                  ...
                  <div className="ml-4 shrink-0 text-right">
                    <span className={`font-display text-xl font-bold ${getScoreColor(attempt.percentage ?? 0)}`}>
                      {attempt.percentage?.toFixed(1)}%
                    </span>
                    <p className="text-xs text-ink-400 font-mono">
                      {attempt.correct_count}C / {attempt.incorrect_count}W
                    </p>
                  </div>
                </div>
              ))}
